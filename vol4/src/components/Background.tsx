...
export default function Background() {
  const mat = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        col += vec3(0.04, 0.03, 0.10) * exp(-cd * cd * 4.0) * (uGlowStr / 0.04);
        ...
        if (sr > 0.92) {
          ...
          float brightness = (tw * 0.5 + 0.5) * 0.7 * uStarBright;
        ...
        if (sr2 > 0.96) {
          ...
          col += vec3(0.75, 0.72, 0.95) * star2 * (tw2 * 0.3 + 0.7) * 0.9 * uStarBright;
        }
  ...
  useFrame((_, delta) => {
    ...
    mat.uniforms.uStarBright.value = s.starBrightness;
    mat.uniforms.uGlowStr.value = s.bgGlowStrength;
  });
